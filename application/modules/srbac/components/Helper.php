...
class Helper {
	...
	{
		$criteria = new CDbCriteria(array('scopes' => array('type' => CAuthItem::TYPE_ROLE), 'order' => 't.name ASC'));
		if($userid)
		{
			$criteria->with = array('assignments' => array('on' => 'assignments.user_id=:user_id', 'params' => array(':user_id' => $userid)));
			$criteria->addColumnCondition(array('assignments.user_id' => null));
		}
		else
		{
	 ...
	 * @param mixed $item Identifier of the role
	 * @return array An array of tasks(AuthItems) assigned to the role
	 */
	public static function getRoleAssignedTasks($item)
	{
		$criteria = new CDbCriteria(array('scopes' => array('type' => CAuthItem::TYPE_TASK), 'order' => 't.name ASC'));
		if($item)
		{
			$itemId = Yii::app()->getAuthManager()->getItemIdentity($item);
			$criteria->with = 'parents';
			$criteria->addColumnCondition(array('parents.type' => CAuthItem::TYPE_ROLE, 'parents.'.$itemId[0] => $itemId[1]));
		}
		return AuthItem::model()->findAll($criteria);
	}

	/**
	 * Return the tasks not assigned to a role, or all the tasks if no role is provided
	 * @param mixed $item Identifier of the role
	 * @return array An array of tasks(AuthItems) not assigned to the role
	 */
	public static function getRoleNotAssignedTasks($item)
	{
		$criteria = new CDbCriteria(array('scopes' => array('type' => CAuthItem::TYPE_TASK), 'order' => 't.name ASC'));
		if($item)
		{
			$itemId = Yii::app()->getAuthManager()->getItemIdentity($item);
			$criteria->with = array('parents' => array('scopes' => array('type' => CAuthItem::TYPE_ROLE), 'on' => 'parents.'.$itemId[0].'=:id', 'params' => array(':id' => $itemId[1])));
			$criteria->addColumnCondition(array('parents.id' => null));
		}
		return AuthItem::model()->findAll($criteria);
	}
	 ...
	 */
	public static function getTaskAssignedOpers($item)
	{
		$criteria = new CDbCriteria(array('scopes' => array('type' => CAuthItem::TYPE_OPERATION), 'order' => 't.name ASC'));
		if($item)
		{
			$itemId = Yii::app()->getAuthManager()->getItemIdentity($item);
			$criteria->with = 'parents';
			$criteria->addColumnCondition(array('parents.type' => CAuthItem::TYPE_TASK, 'parents.'.$itemId[0] => $itemId[1]));
		}
		return AuthItem::model()->findAll($criteria);
	}
	 ...
	 */
	public static function getTaskNotAssignedOpers($item) {
		$criteria = new CDbCriteria(array('scopes' => array('type' => CAuthItem::TYPE_OPERATION), 'order' => 't.name ASC'));
		if($item)
		{
			$itemId = Yii::app()->getAuthManager()->getItemIdentity($item);
			$criteria->with = array('parents' => array('scopes' => array('type' => CAuthItem::TYPE_TASK), 'on' => 'parents.'.$itemId[0].'=:id', 'params' => array(':id' => $itemId[1])));
			$criteria->addColumnCondition(array('parents.id' => null));
		}
		return AuthItem::model()->findAll($criteria);
	}
	...
	public static function publishCss($css, $forcePublish = false) {
		if (Yii::app()->getRequest()->getIsAjaxRequest() && !$forcePublish) {
			return true;
		}
		//Search in default Yii css directory
		$cssFile = Yii::getPathOfAlias("webroot.css") . DIRECTORY_SEPARATOR . $css;
		if (is_file($cssFile) && !Yii::app()->clientScript->isCssFileRegistered(Yii::app()->getRequest()->baseUrl . "/css/" . $css)) {
			$cssUrl = Yii::app()->getRequest()->baseUrl . "/css/" . $css;
