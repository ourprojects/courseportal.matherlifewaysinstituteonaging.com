...
class PhpbbModule extends CWebModule
{
	...
	/**
	 * The name of the user class used by your application
	 * @var string
	 */
	public $userClass = 'User';

	/**
	 * The name of the column containing a user's unique name
	 * @var string
	 */
	public $userName = 'username';

	/**
	 * The name of the database connection component
	 * @var string
	 */
	...
	public static function getInstance()
	{
		...
		{
			self::$_module = self::_findModule(Yii::app());
			if(self::$_module === null)
			{
				throw new CException(Yii::t('srbac', 'Unable to locate module. Make sure the module is configured and named {name}.', array('{name}' => self::MODULE_NAME)));
			}
		}
		return self::$_module;
	}
	
	private static function _findModule($parentModule)
	{
		if(($module = $parentModule->getModule(self::MODULE_NAME)) === null)
		{
			foreach($parentModule->getModules() as $module => $config)
			{
				if(($module = $parentModule->getModule($module)) !== null && ($module = self::_findModule($module)) !== null)
				{
					break;
				}
			}
	...
	public function getDbConnection()
	{
		if(!isset($this->_db))
		{
			...
			{
				throw new CException(Yii::t(self::ID, 'PhpbbModule.connectionID is invalid. Please make sure "{id}" refers to a valid database application component.',
	...
	public function init()
	{
		...
		if(!isset($this->path))
		{
			throw new CException(Yii::t(self::ID, 'Path to phpBB forum has not been set.'));
		}
