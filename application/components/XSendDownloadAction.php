...
class XSendDownloadAction extends CAction {
		...
		if($this->_filePath === null) {
			...
			if(empty($this->_filePath)) {
				if(!empty($this->defaultFile))
					...
					throw new CHttpException(400, t('A file to download was not specified.'));
			}
		}
	...
	protected function resolveFile($filePath) {
		...
		if(is_file($file)) {
			...
			throw new CHttpException(404, t('File "{file}" does not exist.', 
				array('{file}' => $filePath)));
		}
	}
	...
	public function run() {
		...
		if(!$event->handled) {
			Yii::app()->getRequest()->xSendFile($this->file, array(
