<?php
$action = $course->getIsNewRecord() ? 'Create' : 'Update';

$this->breadcrumbs = array(
		t('Admin') => $this->createUrl('admin'), 
		t('Courses') => $this->createUrl('course'), 
		t('Course ' . $action)
);
?>
...
<?php if(!$course->getIsNewRecord()): ?>
	<div id="objectives">
		<h2><?php echo TranslateModule::t('Objectives'); ?></h2>
		<?php 
		...
		$this->widget('zii.widgets.grid.CGridView', array(
			...
			'columns' => array(
		        ...
		        array(
		            'class' => 'CButtonColumn',
		            'template' => '{delete}',
		            'deleteButtonUrl' => 'Yii::app()->getController()->createUrl("courseObjectiveDelete", array("id" => $data->id))',
		        )
			),
		)); ?>
		...
		<div class="form">
			...
			<div class="row submit">
				<?php echo CHtml::submitButton(t('Add Objective')); ?>
			</div>
			<?php $this->endWidget(); ?>
		</div>
	</div>
